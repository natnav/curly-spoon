...
const VIBE_MAP = {
  'rainy day':        ['indie folk acoustic', 'lo-fi melancholy', 'soft piano sad', 'mellow reflective indie'],
  'late night drive': ['synthwave drive',     'dark ambient night', 'atmospheric indie', 'nocturnal electronic'],
  'late night':       ['synthwave',            'dark ambient',       'night lo-fi',       'atmospheric chill'],
  'gym hype':         ['workout rap hype',     'high energy edm',    'intense trap',      'pump up hip hop'],
  'hype':             ['hype rap',             'trap banger',        'high energy edm',   'aggressive hip hop'],
  'chill':            ['lo-fi hip hop',        'chillhop',           'soft r&b chill',    'ambient chill'],
  'summer':           ['summer pop upbeat',    'beach feel good',    'tropical pop',      'indie summer'],
  'heartbreak':       ['sad pop breakup',      'emotional ballad',   'slow heartbreak',   'melancholy indie'],
  ...
  'focus':            ['lo-fi study',          'instrumental focus', 'ambient piano',     'concentration music'],
  'study':            ['lo-fi study',          'instrumental study', 'ambient focus',     'soft piano study'],
  'romantic':         ['romantic r&b',         'love songs acoustic','slow dance pop',    'soft romantic indie'],
  'nostalgic':        ['80s pop classic',      'retro synthpop',     'classic rock hits', 'oldies feel good'],
  'angry':            ['metal aggressive',     'hard rock intense',  'rage rap',          'punk rock'],
  'sleep':            ['sleep ambient',        'calm piano',         'soft lullaby',      'relaxing nature'],
  'road trip':        ['road trip rock',       'indie driving',      'classic rock',      'upbeat pop travel'],
};

const SLANG_MAP = {
  'turn up':          ['energetic party',      'dance hype',           'edm banger',             'club rap'],
  'in my feels':      ['emotional sad indie',  'introspective slow',   'heartbreak ballad',      'melancholy acoustic'],
  'main character':   ['cinematic indie pop',  'confident empowering', 'cool alternative',       'bold anthems'],
  'gaming':           ['video game soundtrack', 'game OST',             'nintendo music',         'video game music instrumental'],
  'video games':      ['video game soundtrack', 'game OST',             'nintendo music',         'video game music instrumental'],
  'game':             ['video game soundtrack', 'game OST instrumental','game music',             'retro game music'],
  'nintendo':         ['nintendo music',        'video game soundtrack','game OST',               'super mario music'],
  'understood the assignment': ['bold confident pop', 'powerful hip hop', 'empowering dance',    'attitude banger'],
  'villain arc':      ['dark menacing',        'aggressive electronic','dark hip hop intense',   'cinematic dark'],
  'hot girl summer':  ['confident summer pop', 'upbeat dance',         'empowering feel good',   'attitude pop'],
  'serotonin':        ['happy uplifting pop',  'feel good bright',     'joyful indie',           'cheerful'],
  'night owl':        ['late night synthwave', 'dark ambient lo-fi',   'nocturnal chill',        'midnight electronic'],
  'therapy session':  ['sad acoustic slow',    'emotional introspective','healing indie',         'vulnerable pop'],
  'brat':             ['attitude pop edgy',    'bold dance',           'confident aggressive pop','unapologetic'],
  'cottagecore':      ['folk acoustic gentle', 'pastoral indie soft',  'nature dreamy',          'cozy acoustic'],
  'dark academia':    ['dark classical moody', 'intellectual indie',   'atmospheric orchestral', 'moody piano'],
  'lowkey':           ['understated lo-fi',    'soft acoustic mellow', 'quiet indie',            'subtle chill'],
  'hyperpop':         ['hyperpop electronic',  'glitchy pop edgy',     'maximalist pop',         'alternative bubbly'],
  'vibe':             ['lo-fi chill mellow',   'soft r&b',             'ambient groove',         'smooth indie'],
  'emo':              ['emo pop punk',         'emotional alternative', 'sad rock angst',         'post-hardcore'],
  'heartcore':        ['raw emotional indie',  'vulnerable acoustic',  'slow heartbreak',        'sad ballad'],
};
...
function getKeywords(vibe) {
  ...
  const result = [...expanded, normalized].filter(Boolean);
...
function normalizeTitle(title = '') {
  return title
    .toLowerCase()
    .replace(/\s*[\(\[][^\)\]]*(remix|live|version|edit|acoustic|remaster|remastered|instrumental|cover|demo|mix|radio|feat\.?|ft\.?)[^\)\]]*[\)\]]/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
app.post('/api/playlist', async (req, res) => {
  ...
  try {
    ...
    if (varied.length === 0) {
      return res.status(404).json({ error: 'No songs found for this vibe. Try something else!' });
    }
    ...
    const label = vibe.trim().charAt(0).toUpperCase() + vibe.trim().slice(1).toLowerCase();
